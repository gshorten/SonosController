...
There are Classes for using an Adafruit RGB Rotary Encoder - has a push button, 2bit grey code rotary encoder,
and RGB LED.
The rotary encoder class is based on algorithm by Ben Buxton.  See his notes below.
I made minor modifications to include class for the LED on the
...
class PushButton:
    ...
    def __init__(self, pin, callback, short=1):
        self.pin = pin                  # GPIO pin
        GPIO.setup(pin, GPIO.IN)
        self.callback = callback        # callback from method that is called when button is pushed
        GPIO.add_event_detect(self.pin, GPIO.BOTH, callback=self.button_press, bouncetime=5)
